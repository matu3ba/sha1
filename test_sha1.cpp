 ...
 */
void test_file(string const &filename);

static void compare(string const &result, string const &expected) {
  string const &state = (result == expected) ? "OK" : "Failure";
...
static void test_standard() {
  ...
  cout << "Test:     "
          ...
          "rstu"
       << endl;
  checksum.update("abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnop"
                  "qrstnopqrstu");
...
static void test_slow() {
  // https://www.di-mgt.com.au/sha_testvectors.html

  SHA1 checksum;

  cout << endl;
  cout << "Test:     16,777,216 repititions of abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmno"
       << endl;
...
int main(int argc, char const *argv[]) {
  bool const slow = (argc == 2 && std::string("--slow") == argv[1]);
